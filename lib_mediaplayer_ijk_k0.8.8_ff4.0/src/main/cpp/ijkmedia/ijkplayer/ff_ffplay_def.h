...
#define MAX_QUEUE_SIZE (4 * 1024 * 1024) // 4M
...
#define SDL_AUDIO_MIN_BUFFER_SIZE (4 * 1024) // 4KB
...
inline static void
ffp_notify_msg4(FFPlayer *ffp, int what, int arg1, int arg2, void *obj, int obj_len) {
...
inline static const char *ffp_get_error_string(int error) {
    switch (error) {
        ...
        case AVERROR(EAGAIN):
            return "AVERROR(EAGAIN)";       // 35
        case AVERROR(ETIMEDOUT):
            return "AVERROR(ETIMEDOUT)";    // 60
        case AVERROR_EOF:
            return "AVERROR_EOF";
        case AVERROR_EXIT:
            return "AVERROR_EXIT";
