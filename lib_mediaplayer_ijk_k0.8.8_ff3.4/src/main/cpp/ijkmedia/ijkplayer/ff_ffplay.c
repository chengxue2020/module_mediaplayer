...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (is->is_video_high_fps) {
            avctx->skip_frame       = FFMAX(avctx->skip_frame, AVDISCARD_ALL);
            avctx->skip_loop_filter = FFMAX(avctx->skip_loop_filter, AVDISCARD_ALL);
            avctx->skip_idct        = FFMAX(avctx->skip_loop_filter, AVDISCARD_ALL);
        }
