...
import lib.kalu.mediaplayer.ui.old.surface.VideoTextureView;
...
public class VideoMediaPlayer {
    ...
    public void openMediaPlayer() {
        ...
        try {
            ...
            // 播放准备中
            videoPlayer.setCurrentState(PlayerType.StateType.STATE_PREPARING);
            // 控制器，更新不同的播放状态的UI
    ...
    private IMediaPlayer.OnCompletionListener mOnCompletionListener =
            new IMediaPlayer.OnCompletionListener() {
                ...
                public void onCompletion(IMediaPlayer mp) {
                    videoPlayer.setCurrentState(PlayerType.StateType.STATE_COMPLETED);
                    videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
    ...
    private IMediaPlayer.OnPreparedListener mOnPreparedListener = new IMediaPlayer.OnPreparedListener() {
        ...
        public void onPrepared(IMediaPlayer mp) {
            videoPlayer.setCurrentState(PlayerType.StateType.STATE_PREPARED);
            videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
    ...
    private IMediaPlayer.OnErrorListener mOnErrorListener = new IMediaPlayer.OnErrorListener() {
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
            ...
            if (what != -38 && what != -2147483648 && extra != -38 && extra != -2147483648) {
                videoPlayer.setCurrentState(PlayerType.StateType.STATE_ERROR);
                videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
            }
    ...
    private IMediaPlayer.OnInfoListener mOnInfoListener = new IMediaPlayer.OnInfoListener() {
        ...
        public boolean onInfo(IMediaPlayer mp, int what, int extra) {
            if (what == IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                // 播放器开始渲染
                videoPlayer.setCurrentState(PlayerType.StateType.STATE_PLAYING);
                videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
                VideoLogUtils.d("listener---------onInfo ——> MEDIA_INFO_VIDEO_RENDERING_START：STATE_PLAYING");
            } else if (what == IMediaPlayer.MEDIA_INFO_BUFFERING_START) {
                // MediaPlayer暂时不播放，以缓冲更多的数据
                if (videoPlayer.getCurrentState() == PlayerType.StateType.STATE_PAUSED ||
                        videoPlayer.getCurrentState() == PlayerType.StateType.STATE_BUFFERING_PAUSED) {
                    videoPlayer.setCurrentState(PlayerType.StateType.STATE_BUFFERING_PAUSED);
                    VideoLogUtils.d("listener---------onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PAUSED");
                } else {
                    videoPlayer.setCurrentState(PlayerType.StateType.STATE_BUFFERING_PLAYING);
                    VideoLogUtils.d("listener---------onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PLAYING");
                }
                videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
            } else if (what == IMediaPlayer.MEDIA_INFO_BUFFERING_END) {
                // 填充缓冲区后，MediaPlayer恢复播放/暂停
                if (videoPlayer.getCurrentState() == PlayerType.StateType.STATE_BUFFERING_PLAYING) {
                    videoPlayer.setCurrentState(PlayerType.StateType.STATE_PLAYING);
                    videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
                    VideoLogUtils.d("listener---------onInfo ——> MEDIA_INFO_BUFFERING_END： STATE_PLAYING");
                }
                if (videoPlayer.getCurrentState() == PlayerType.StateType.STATE_BUFFERING_PAUSED) {
                    videoPlayer.setCurrentState(PlayerType.StateType.STATE_PAUSED);
                    videoPlayer.getController().onPlayStateChanged(videoPlayer.getCurrentState());
